...
def successfulDBUpdate() {
    ...
    sh '''if [ $(find ${DB_PATH}/daily -type f -atime -14 -name cosima_master_????-??-??.db | wc -l) -gt 7 ]; then
              find ${DB_PATH}/daily -type f -atime +14 -name "cosima_master_????-??-??.db" -or -name "cosima_master_????-??-??.db-journal" -delete
          fi'''
}
